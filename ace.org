...
** Completion style (orderless.el & pinyinlib.el)

#+begin_src emacs-lisp
(use-package pinyinlib
  :config
  (require 'ace-pinyin)
  (setq pinyinlib--simplified-char-table ace/pinyinlib-xiaohe))
...
(use-package orderless
  ...
  (defun ace/orderless-pinyin-dispatcher (pattern _index _total)
    ...
    (when (string-prefix-p "`" pattern)
      `(pinyinlib-build-regexp-string . ,(substring pattern 1))))
...
(use-package ace-embark
  ...
  (let ((map embark-collect-mode-map))
    ...
    (define-key map (kbd "M-n") #'ace/embark-completions-act-next)
    (define-key map (kbd "M-p") #'ace/embark-completions-act-previous)))
#+end_src
...
(use-package ace-completing
  :commands (ace/completing-file-in-user-dirs)
  :straight nil
  :config
...
(use-package ace-ibuffer :straight nil)
#+end_src
