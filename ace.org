...
** Custom primitive commands (ace-atom.el)

#+begin_src emacs-lisp
(use-package ace-atom
  :straight nil
  :after evil
  :bind
  (:map evil-insert-state-map
    ("<C-i>" . ace/atom-backward-char)
    ("C-o" . ace/atom-forward-char)
   :map minibuffer-local-map
    ("<C-i>" . ace/atom-backward-char)
    ("C-o" . ace/atom-forward-char)))
#+end_src

** Async facilities (async.el)
...
(use-package ace-simple
  ...
  :bind
  (("s-n" . ace/simple-winmove-r/d)
   ("s-p" . ace/simple-winmove-l/u)
   ("s-m" . ace/simple-monocle)
   ("s-q" . ace/simple-kill-window-current)
...
(use-package evil
  ...
  (evil-define-key 'insert global-map (kbd "C-g")    #'evil-normal-state)
...
(use-package evil-collection
  :after evil
  :config
  (evil-collection-init '(rg helpful help)))
#+end_src
...
- If =electric-pair-mode= is enabled (which I might do manually), insert quotes
   ...
   the cursor.
- To get those numbers, evaluate =(string-to-char CHAR)= where CHAR is the one
   you are interested in.  For example, get the literal tab's character with
   =(string-to-char "\t")=.
...
(use-package consult
  ...
  (with-eval-after-load 'evil
    (evil-define-key '(normal visual insert) global-map (kbd "C-/") #'consult-isearch)
    (evil-define-key '(normal visual insert) global-map (kbd "C-l") #'consult-line)
    (evil-define-key '(normal visual) global-map (kbd "C-f") #'consult-outline))
  ...
  (transient-define-prefix ace/leader-consult ()
    ...
    ["Consult"
     [("g" "Grep" consult-grep)
      ...
      ("l" "isearch" consult-line)
      ("o" "isearch" consult-outline)
...
(use-package ace-completing
  :straight nil
  :bind
  (("s-f" . ace/completing-buffers-major-mode)
   ("C-x d" . ace/completing-word-ispell)))
#+end_src
...
(use-package windmove
  ...
  :bind
  ("s-h" . (lambda () (interactive) (split-window-right) (windmove-right)))
  ("s-v" . (lambda () (interactive) (split-window-below) (windmove-down))))
#+end_src
...
(use-package ibuffer
  :bind
  ("s-i" . (lambda (a) (interactive "P") (if a (ibuffer) (next-buffer))))
  ("s-o" . (lambda (a) (interactive "P") (if a (ibuffer) (previous-buffer))))
  :config
...
* Eshell
** Basic (eshell.el & ace-eshell.el)

#+begin_src emacs-lisp
(use-package ace-eshell
  :straight nil
  :config
  (setq eshell-banner-message "")
  (setq eshell-aliases-file (concat ace/init-dot-repo "eshell/aliases"))
  (add-hook 'eshell-first-time-mode-hook 'ace/eshell-configure)
  (with-eval-after-load 'esh-opt
    (setq eshell-destroy-buffer-when-process-dies t)
    (setq eshell-visual-commands '("htop" "zsh" "nvim")))
  (define-key global-map (kbd "s-e") 'ace/eshell-toggle)
  (evil-define-key '(normal insert visual) eshell-mode-map (kbd "C-f") 'consult-outline)
  (evil-define-key '(normal insert visual) eshell-mode-map (kbd "C-r") 'consult-history)
  (evil-define-key '(normal insert visual) eshell-mode-map (kbd "C-u") 'eshell-kill-input)
  (evil-define-key '(normal insert visual) eshell-mode-map (kbd "C-n") 'eshell-next-prompt)
  (evil-define-key '(normal insert visual) eshell-mode-map (kbd "C-p") 'eshell-previous-prompt)
  (define-key eshell-mode-map (kbd "s-<backspace>") 'ace/eshell-clear-buffer)
  (define-key eshell-mode-map (kbd "s-c") 'ace/eshell-new)
  (define-key eshell-mode-map (kbd "s-i") 'ace/eshell-next)
  (define-key eshell-mode-map (kbd "s-o") 'ace/eshell-prev))
#+end_src
...
(use-package esh-autosuggest
  ...
  :config
  (let ((map esh-autosuggest-active-map))
    (define-key map (kbd "C-f") 'esh-autosuggest-complete-word)
    (define-key map (kbd "C-<return>") 'company-complete-selection))
...
** User directory (ace-files.el)

#+begin_src emacs-lisp
(use-package ace-files
  :straight nil)
#+end_src
...
(use-package lf
  ...
  (transient-define-prefix ace/lf-go ()
    ...
    ["Directory"
     ...
     ("c" "Code" (lambda () (interactive) (lf-find-file "~/Code")))
  ...
  (define-key lf-mode-map (kbd "s-e")
    (lambda ()
      (interactive)
      (let ((ace/eshell-position '((window-height . 0.35) (side . bottom))))
        (ace/eshell-toggle t))))
...
(transient-define-prefix ace/leader-eval ()
  "Eval (elisp) commands"
  ["Eval"
   [("l" "Eval last s-expression" eval-last-sexp)
...
(use-package lsp-mode
  ...
  (dolist (mode '(sh lua haskell web typescript ace/vue))
    (add-hook (intern (format "%s-mode-hook" mode)) #'lsp-deferred))
...
(bind-keys
  ...
  ("s-z" . window-toggle-side-windows)
  ("s-<backspace>" . (lambda () (interactive) (let ((inhibit-read-only t)) (erase-buffer)))))
#+end_src
...
(transient-define-prefix ace/leader-tab ()
  "Util commands"
  ["Tab"
   [("t" "New/Select tab" ace/tab-select-tab-dwim)
    ("x" "Close tab" tab-close)
    ("l" "Toggle tab line" ace/tab-tab-bar-toggle)]])
...
(transient-define-prefix ace/leader-window ()
  ...
  ["Window"
   [("f" "Flop frame windows" flop-frame)
    ...
    ("=" "Balance windows size" balance-windows-area)
    ("t" "toggle side windows" window-toggle-side-windows)
    ("i" "zoom In window" evil-window-increase-width)
    ("o" "zoom Out window" evil-window-decrease-width)
...
;; ("u"   "Messages" ace/atom-display-message)
